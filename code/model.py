...
class Encoder(nn.Module):
    def __init__(self, config):
        ...
        self.rnn = nn.LSTM(
            ...
            hidden_size=(
                self.rnn_size // 2 if self.bidirectional_encoder else self.rnn_size
            ),
        ...
        if self.bidirectional_encoder:
            ...
            for i in range(2):
                temp_new_hidden = (
                    ...
                    .transpose(1, 2)
                    .contiguous()
                    .view(self.num_layers, hidden[i].size(1), hidden[i].size(2) * 2)
                )
...
class Decoder(nn.Module):
    def __init__(self, config):
        ...
        self.embedding = nn.Embedding(
            num_embeddings=config.vocab_size, embedding_dim=self.rnn_size
        )
        self.rnn = nn.LSTM(self.rnn_size, self.rnn_size, self.num_layers)
    ...
    def forward(self, x, hidden, full_encoder_ouput):
        ...
        log_softmax_output = F.log_softmax(output.permute(2, 1, 0)).permute(2, 1, 0)
        return (log_softmax_output.permute(1, 0, 2), hidden, dict())

    def init_hidden(self, batch_size):
        weight = next(self.parameters()).data

        h = Variable(weight.new(self.num_layers, batch_size, self.rnn_size).zero_())
        c = Variable(weight.new(self.num_layers, batch_size, self.rnn_size).zero_())
        return h, c
...
class Attn(nn.Module):
    def __init__(self, config):
        ...
        if self.method == "dot":
            ...
            hidden_size_mult = 2
            self.attn = nn.Linear(self.hidden_size * hidden_size_mult, self.hidden_size)
...
class AttnDecoder(nn.Module):
    def __init__(self, config):
        ...
        self.rnn = nn.LSTM(self.rnn_size, self.rnn_size, self.num_layers)
        self.fc1 = nn.Linear(self.rnn_size * 2, config.vocab_size)
        self.softmax = nn.LogSoftmax()
    ...
    def forward(self, x, hidden, full_encoder_output):
        ...
        context = torch.bmm(
            attn_weights.t().unsqueeze(1), full_encoder_output.transpose(0, 1)
        ).transpose(0, 1)
        ...
        return (log_softmax_output.permute(1, 0, 2), hidden, {"attn": attn_weights})

    def init_hidden(self, batch_size):
        weight = next(self.parameters()).data

        h = Variable(weight.new(self.num_layers, batch_size, self.rnn_size * 2).zero_())
        c = Variable(weight.new(self.num_layers, batch_size, self.rnn_size * 2).zero_())
        return h, c
